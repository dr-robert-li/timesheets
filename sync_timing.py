...
def main():
    ...
    venv_python = os.path.join(subfolder, "venv", "bin", "python")
    sync_script = os.path.join(subfolder, "sync_timing_to_clockify.py")

    # Check if venv exists
    if not os.path.exists(venv_python):
        print(f"Error: Virtual environment not found at {venv_python}")
        print("Please run: cd timing-clockify && python3 -m venv venv && source venv/bin/activate && pip install requests")
        sys.exit(1)
